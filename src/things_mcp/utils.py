...
class DeadLetterQueue:
    ...
    def retry_all(self):
        ...
        # URL scheme functionality removed - DLQ retry not implemented for AppleScript
        logger.warning("DLQ retry functionality disabled - URL scheme removed")

        return {
            "success": False,
            "retried": 0,
            "failed": len(self.queue),
            "message": "Retry functionality disabled after URL scheme removal"
        }
...
# Auth token functionality removed - AppleScript doesn't require authentication
